...
def build_shelf_cache(bookshelf_books):
    ...
    # 从 booksAndArchives 提取（主要数据来源）
    for item in bookshelf_books.get("booksAndArchives", []):
...
def parse_read_info(read_info):
    ...
    start_reading_time = book_data.get("startReadingTime", 0)
    update_time = book_data.get("updateTime", 0)
    ...
    return {
        ...
        "beginReadingDate": start_reading_time,
...
def prepare_book_data(book_id, shelf_book, read_info, archive_name=None):
    ...
    """
    # 合并数据（阅读信息优先级更高）
    book = {**shelf_book, **read_info}
    
    # 添加书架分类
    if archive_name:
        book["书架分类"] = archive_name
    
    # 计算阅读状态
    marked_status = book.get("markedStatus", 1)
    if marked_status == 4:
        status = "已读"
    ...
    # 计算阅读进度（转换为 0-1 的小数）
    book["阅读进度"] = (book.get("readingProgress") or 0) / 100
    ...
    # 处理阅读时间日期
    begin_date = book.get("beginReadingDate") or 0
    last_date = book.get("lastReadingDate") or 0
    
    # 计算阅读天数
    if begin_date and last_date:
        try:
            days = pendulum.from_timestamp(last_date).diff(
                pendulum.from_timestamp(begin_date)
            ).in_days()
            book["阅读天数"] = max(1, days)  # 至少 1 天
        except:
            book["阅读天数"] = 0
    else:
        book["阅读天数"] = 0
...
def sync_reading_time(page_id, read_info, notion_helper):
    """
    同步阅读时间数据到阅读记录数据库
    
    注意：新 API 没有每日阅读明细，只有总时长
    """
    # TODO: 如果需要详细阅读数据，可能需要调用其他 API
    pass
...
def main():
    ...
    for i, (book_id, book_data) in enumerate(new_books, 1):
        try:
            ...
            # 同步阅读时间（使用缓存）
            if book_id in read_info_cache:
                sync_reading_time(page_id, read_info_cache[book_id], notion_helper)
